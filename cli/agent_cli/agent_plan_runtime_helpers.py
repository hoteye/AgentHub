...
from cli.agent_cli import agent_plan_facade_runtime
from cli.agent_cli import agent_plan_runtime
    ...
    *,
    ...
    if agent._planner is not None:
        ...
        try:
            ...
            success_diagnostics.setdefault("observed_elapsed_ms", int((time.perf_counter() - planner_started_at) * 1000))
            ...
            return agent._intent_with_protocol_path(
                intent,
                kind="provider_replay_loop" if agent._planner_is_replay_runtime() else "provider_loop",
                source="provider",
                provider_used=True,
            ...
            failure_diagnostics.setdefault("observed_elapsed_ms", int((time.perf_counter() - planner_started_at) * 1000))
    ...
    if live_web_query or explicit_url:
        ...
        if fallback_intent is not None:
            ...
            if runtime_error:
                ...
                protocol_diagnostics = dict(getattr(fallback_intent, "protocol_diagnostics", {}) or {})
                ...
                commentary_lines = []
                existing_commentary = str(getattr(fallback_intent, "commentary_text", "") or "").strip()
