...
class InMemoryGatewayStateStore:
    ...
    def get_action_request(self, action_id: str) -> Optional[ActionRequest]:
    ...
    def get_approval_ticket(self, approval_id: str) -> Optional[ApprovalTicket]:
    ...
    def list_approval_tickets(
        ...
        *,
        ...
        if normalized_status:
            items = [item for item in items if str(item.status or "").strip().lower() == normalized_status]
    ...
    def list_audit_records(
        ...
        *,
        ...
        if normalized_trace_id:
            items = [item for item in items if item.trace_id == normalized_trace_id]
        if normalized_stage:
            items = [item for item in items if str(item.stage or "").strip().lower() == normalized_stage]
        if normalized_status:
            items = [item for item in items if str(item.status or "").strip().lower() == normalized_status]
        if normalized_event_id:
            items = [item for item in items if str(item.event_id or "").strip() == normalized_event_id]
        if normalized_workflow_run_id:
            items = [item for item in items if str(item.workflow_run_id or "").strip() == normalized_workflow_run_id]
        if normalized_action_id:
            items = [item for item in items if str(item.action_id or "").strip() == normalized_action_id]
        if normalized_approval_id:
            items = [item for item in items if str(item.approval_id or "").strip() == normalized_approval_id]
...
class JsonlGatewayStateStore(InMemoryGatewayStateStore):
    def __init__(self, base_dir: str | Path) -> None:
        ...
        self._files = {key: self.base_dir / filename for key, filename in _GATEWAY_JSONL_FILENAMES.items()}
        self._load_existing()

    @classmethod
    def default(cls) -> "JsonlGatewayStateStore":
    ...
    def _load_existing(self) -> None:
        loaders = {
            "events": lambda payload: super(JsonlGatewayStateStore, self).save_event(gateway_event_from_mapping(payload)),
            "workflow_runs": lambda payload: super(JsonlGatewayStateStore, self).save_workflow_run(workflow_run_from_mapping(payload)),
            "action_requests": lambda payload: super(JsonlGatewayStateStore, self).save_action_request(action_request_from_mapping(payload)),
            "approval_tickets": lambda payload: super(JsonlGatewayStateStore, self).save_approval_ticket(approval_ticket_from_mapping(payload)),
            "audit_records": lambda payload: super(JsonlGatewayStateStore, self).append_audit_record(audit_record_from_mapping(payload)),
        }
        for key, target in self._files.items():
