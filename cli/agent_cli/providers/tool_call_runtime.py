...
from . import anthropic_edit_tool_specs, tool_call_runtime_helpers
...
def runtime_tool_call_command(
    ...
    *,
    ...
    if name == "spawn_agent":
        if _uses_legacy_spawn_agent_payload(arguments):
            task = str(
        ...
        items = _normalized_collab_items(arguments.get("items"))
    ...
    if name in {"wait", "wait_agent"}:
        ...
        timeout_ms = arguments.get("timeout_ms")
        if timeout_ms is not None:
            command += f" --timeout-ms {_quote_value(timeout_ms, quote_arg_fn)}"
        reason = str(arguments.get("reason") or "").strip()
        if reason:
            command += f" --reason {quote_arg_fn(reason)}"
        if "wait_required" in arguments and arguments.get("wait_required") is not None:
            command += f" --wait-required {quote_arg_fn(str(bool(arguments.get('wait_required'))).lower())}"
        return command
