...
def _explicit_function_output_item_events(
    event: ToolEvent, *, item_events: list[dict[str, Any]]
) -> list[dict[str, Any]]:
...
def single_event_result(
    ...
    *,
    ...
    if resolved_tool_name.startswith("shell") or resolved_tool_name in {
        "exec_command",
        "write_stdin",
    }:
        item_events = shell_tool_call_item_events(
            event,
            command=str(
                normalized_arguments.get("command") or (event.payload or {}).get("command") or ""
            ).strip()
            or None,
        )
    else:
...
def error_result(
    ...
    *,
    ...
    message = (
        str((event.payload or {}).get("error") or event.summary or "").strip() or "tool failed"
    )
