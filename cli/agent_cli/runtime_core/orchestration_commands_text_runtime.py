...
def orchestrate_created_text(payload: dict[str, Any]) -> str:
...
def orchestrate_confirmation_text(payload: dict[str, Any]) -> str:
    ...
    if status == "confirmed":
        ...
        created_text = orchestrate_created_text(dict(payload.get("created_run") or {}))
        dispatch_text = orchestrate_dispatch_text(dict(payload.get("dispatch_run") or {}))
    ...
    if status == "interactive_unavailable":
        lines = [
            ...
            orchestration_preview_summary(preview),
    ...
    if preview:
        lines.append(orchestration_preview_summary(preview))
    return "\n\n".join(lines)


def orchestrate_dispatch_text(payload: dict[str, Any]) -> str:
...
def orchestrate_progress_text(payload: dict[str, Any]) -> str:
    synced_cards = [
...
def orchestration_preview_request_text(payload: dict[str, Any]) -> str:
    return _orchestration_preview_request_text(payload)


def orchestration_preview_summary(preview: dict[str, Any]) -> str:
    return _orchestration_preview_summary(preview)


def orchestration_preview_full(preview: dict[str, Any]) -> str:
    return _orchestration_preview_full(preview)


def truncate_prompt_text(text: str, *, limit: int = 3600) -> str:
...
def render_continue_pass_summaries(payload: dict[str, Any]) -> str:
...
def orchestrate_continue_text(payload: dict[str, Any]) -> str:
    status = str(payload.get("status") or "").strip()
    ...
    lines.append(f"pass_summaries={render_continue_pass_summaries(payload)}")
    lines.append(
        "synced_cards=" + ",".join(str(item) for item in list(payload.get("synced_card_ids") or []))
        if payload.get("synced_card_ids")
        else "synced_cards=-"
...
def orchestrate_review_text(payload: dict[str, Any], *, applied: bool) -> str:
