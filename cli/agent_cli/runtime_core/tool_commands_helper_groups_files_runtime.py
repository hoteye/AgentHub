...
ToolCommandResult = tuple[str, list[ToolEvent]] | CommandExecutionResult | None
...
def _install_runtime_view_image_capabilities(runtime) -> Callable[[], None]:
    ...
    tools._view_image_detail = reference_view_image_detail(config)
    tools._view_image_input_capable = reference_view_image_input_capable(config)
    ...
    def _restore() -> None:
        if previous_detail is marker:
            ...
            tools._view_image_detail = previous_detail
        if previous_capable is marker:
            ...
            tools._view_image_input_capable = previous_capable
...
def handle_file_list(
    ...
    *,
    ...
    structured = call_structured(
        runtime.tools, "file_list_result", path=parsed["path"], limit=parsed["limit"]
    )
    if structured is not None:
        return structured
    arguments = dict(parsed)
...
def handle_file_search(
    ...
    *,
    ...
    if not query:
        return text_only_result(
            command_usage_text("file_search")
            or "Usage: /file_search <query> [path <dir>] [limit <n>]"
        )
    structured = call_structured(
        runtime.tools, "file_search_result", query, path=parsed["path"], limit=parsed["limit"]
    )
    if structured is not None:
        return structured
    return single_event_result(
...
def handle_file_read(
    ...
    *,
    ...
    if not target_path:
        return text_only_result(
            command_usage_text("file_read")
            or "Usage: /file_read <path> [offset <line>] [limit <n>]"
        )
    structured = call_structured(
        runtime.tools,
        "file_read_result",
...
def handle_office_run(
    ...
    *,
    ...
    if not parsed["positionals"]:
        return text_only_result(
            command_usage_text("office_run") or "Usage: /office_run <skill> <file>"
        )
    structured = call_structured(
        runtime.tools, "office_run_result", parsed["skill_name"], args=parsed["args"]
    )
    if structured is not None:
        return structured
    return single_event_result(
