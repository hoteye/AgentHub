...
def resume_thread(store: "ThreadStore", thread_id: str) -> Dict[str, Any]:
            ...
            for raw_line in handle:
                ...
                if rollout_item.item_type == "response_item":
                    ...
                    if scope == "turn_context":
                        history_item = store._history_item_from_rollout_payload(rollout_item.payload)
                ...
                if rollout_item.item_type == "turn_context":
                    ...
                    for input_item in list(turn_context.items or []):
                        history_item = store._history_item_from_planner_input_item(input_item.item.to_dict())
                        if history_item is None:
                            continue
    ...
    if scoped_environment_history and not list(effective_state.get("environment_context_history") or []):
    ...
    if "workspace_context_snapshot" not in effective_state:
        ...
        if latest_workspace_item is not None:
            ...
            effective_state["workspace_context_snapshot"] = {
                ...
                "docs": [
                    entry for entry in list(metadata.get("docs") or []) if isinstance(entry, dict)
                ],
                "skills": [
                    entry for entry in list(metadata.get("skills") or []) if isinstance(entry, dict)
                ],
            }
    with store._lock, store._connection() as conn:
        ...
        conn.commit()
    planner_fallback_history = list(base_history if compacted_active else ([] if turns else legacy_history))
    return {
        "thread": record,
        "history": history,
