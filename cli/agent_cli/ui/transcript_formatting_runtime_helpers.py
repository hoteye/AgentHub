...
from cli.agent_cli.ui import transcript_formatting_activity_runtime


def format_file_list_lines(event: Any, summary: str, raw: str, *, activity_param_text_fn: Callable[..., str]) -> list[str]:
    del raw
...
def format_file_search_lines(event: Any, summary: str, raw: str, *, activity_param_text_fn: Callable[..., str]) -> list[str]:
    del raw
...
def format_view_image_lines(event: Any, summary: str, raw: str, *, activity_param_text_fn: Callable[..., str]) -> list[str]:
...
def format_web_page_lines(event: Any, summary: str, raw: str, *, activity_param_text_fn: Callable[..., str]) -> list[str]:
...
def format_apply_patch_lines(event: Any, summary: str, raw: str, *, activity_param_text_fn: Callable[..., str]) -> list[str]:
    ...
    count_text = activity_param_text_fn(event, "file_count") or (file_count_line.partition("=")[2].strip() if file_count_line else "")
    path_line = next(
        (
            line
            ...
            and not line.startswith("write_mode=")
        ),
        "",
    )
...
def format_patch_approval_lines(event: Any, summary: str, raw: str, *, activity_param_text_fn: Callable[..., str]) -> list[str]:
    raw_lines = [line.strip() for line in raw.splitlines() if line.strip()]
    approval_id = activity_param_text_fn(event, "approval_id") or (raw_lines[0] if raw_lines else "")
    file_count_line = next((line for line in raw_lines if line.startswith("files=")), "")
    count_text = activity_param_text_fn(event, "file_count") or (file_count_line.partition("=")[2].strip() if file_count_line else "")
    return transcript_formatting_activity_runtime.format_patch_approval_lines(
        summary=summary,
...
def format_shell_approval_lines(event: Any, summary: str, raw: str, *, activity_param_text_fn: Callable[..., str]) -> list[str]:
    raw_lines = [line.strip() for line in raw.splitlines() if line.strip()]
    approval_id = activity_param_text_fn(event, "approval_id") or (raw_lines[0] if raw_lines else "")
    command_text = activity_param_text_fn(event, "command") or (raw_lines[1] if len(raw_lines) >= 2 else "")
    return transcript_formatting_activity_runtime.format_shell_approval_lines(
        summary=summary,
        approval_id=approval_id,
...
def format_action_approval_lines(event: Any, summary: str, raw: str, *, activity_param_text_fn: Callable[..., str]) -> list[str]:
    raw_lines = [line.strip() for line in raw.splitlines() if line.strip()]
    approval_id = activity_param_text_fn(event, "approval_id") or (raw_lines[0] if raw_lines else "")
    lead_text = (
        activity_param_text_fn(event, "task", "summary")
        or (raw_lines[1] if len(raw_lines) >= 2 else "")
    )
    return transcript_formatting_activity_runtime.format_action_approval_lines(
        summary=summary,
        approval_id=approval_id,
        lead_text=lead_text,
        commands=approval_surface_runtime.approval_commands(
...
def format_approval_list_lines(event: Any, summary: str, raw: str, *, activity_param_text_fn: Callable[..., str]) -> list[str]:
    del raw
    return transcript_formatting_activity_runtime.format_approval_list_lines(
        summary=summary,
        count_text=activity_param_text_fn(event, "count"),
...
def format_approval_decision_lines(event: Any, summary: str, raw: str, *, activity_param_text_fn: Callable[..., str]) -> list[str]:
    return transcript_formatting_activity_runtime.format_approval_decision_lines(
        summary=summary,
