...
class RequestUserInputFallbackNoticeTest(unittest.IsolatedAsyncioTestCase):
    @staticmethod
    def _transcript_text(app: AgentCliApp) -> str:
        return app.query_one("#main_log").text

    async def test_fallback_notice_when_presenter_is_missing(self) -> None:
        ...
        original_presenter = getattr(request_user_input_modal, "present_request_user_input", None)
        had_presenter = hasattr(request_user_input_modal, "present_request_user_input")
        if had_presenter:
        ...
        try:
            ...
            if had_presenter:
                setattr(request_user_input_modal, "present_request_user_input", original_presenter)
        ...
        self.assertIn("User input request was cancelled.", transcript)
        self.assertIsNone(runtime.last_response)
        self.assertEqual(app.status_data.get("request_user_input_waiting"), "false")

    async def test_fallback_notice_when_presenter_declines(self) -> None:
        runtime = _RequestUserInputRuntime()
        app = AgentCliApp(runtime=runtime)
