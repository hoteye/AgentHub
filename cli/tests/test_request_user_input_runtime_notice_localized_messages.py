...
class _RequestUserInputRuntime(_BaseRuntime):
    ...
    def handle_prompt(self, text: str, *, attachments=None) -> PromptResponse:
        ...
        return PromptResponse(
            user_text="trigger request_user_input",
...
class RequestUserInputRuntimeNoticeLocalizedMessagesTest(unittest.IsolatedAsyncioTestCase):
    @staticmethod
    ...
    async def test_runtime_notices_use_chinese_copy_when_language_is_explicit_zh_cn(self) -> None:
        ...
        async with app.run_test() as pilot:
            ...
            request_task = asyncio.create_task(
                app._enqueue_runtime_request("trigger zh-cn notice copy", [])
            )
            ...
            self.assertIn("用户已取消输入请求", transcript)
            self.assertEqual(app.status_data.get("request_user_input_waiting"), "false")

    async def test_runtime_notices_keep_legacy_english_when_language_not_explicitly_set(self) -> None:
        runtime = _RequestUserInputRuntime()
        app = AgentCliApp(runtime=runtime)
        app._request_user_input_modal_presenter = (
            lambda *, payload, on_submit, on_cancel: False
        )

        with patch.object(request_user_input_modal, "present_request_user_input", return_value=False):
            async with app.run_test() as pilot:
                await pilot.pause()
                await app._enqueue_runtime_request("trigger default-language notice copy", [])
                await app._wait_for_runtime_idle()
                await pilot.pause()
        ...
        self.assertNotIn("模型请求用户输入", transcript)
