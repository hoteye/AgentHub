...
class OfficeFileTools:
    ...
    def list_skills_result(self) -> CommandExecutionResult:
        ...
        return _structured_result(
            ...
            summary=f"office_skills={int(payload.get('count') or 0)}",
        )
    ...
    def run_skill_result(self, skill_name: str, **kwargs: Any) -> CommandExecutionResult:
        payload = self.run_skill(skill_name, **kwargs)
        return _structured_result(
            tool_name="office_run",
            payload=payload,
            assistant_text="Run Office or PDF skill.",
            arguments={"skill_name": skill_name, "args": dict(kwargs or {}) or None},
            summary=skill_name if bool(payload.get("ok")) else f"{skill_name} failed",
        )
