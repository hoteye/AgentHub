...
def _cell_coordinate(row: int, col: int) -> str:
    return f"{get_column_letter(max(1, col))}{max(1, row)}"


def _collect_openpyxl_merged_cells(sheet: Any) -> List[Dict[str, Any]]:
...
def _collect_xlrd_merged_cells(sheet: Any) -> List[Dict[str, Any]]:
    merged_cells: List[Dict[str, Any]] = []
...
def _sheet_preview(sheet: Any, *, merged_cells: Optional[List[Dict[str, Any]]] = None) -> Dict[str, Any]:
    rows: List[List[str]] = []
    non_empty_rows = 0
